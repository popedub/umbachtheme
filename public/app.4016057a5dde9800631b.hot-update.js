...
const main = async err => {
  ...
  function moverMenu() {
    let menuFake = [...items]; //let menuClone = [...items];

    const menuDesplazado = document.getElementById('fake-ceramica');
    items.forEach(itemMenu => {
      if (!itemMenu.classList.contains('current-menu-item')) {// itemMenu.classList.add('hidden', 'lg:flex')
      } else {//itemMenu.classList.add('flex')
      }
    });
    console.log(items); // menuFake.forEach(element => {
    ...
    menuDesplazado.prepend(...menuFake); // console.log(menuDesplazado)
  }
  ...
  if (items) {
    if (items.length == 2) {
      for (var i = 0; i < items.length; i++) {
        ...
        items[i].classList.add('lg:w-1/2');
      }
    } else {
      for (var i = 0; i < items.length; i++) {
        if (i == 1) {
        ...
        items[i].classList.add('lg:w-1/3');
      }
    }
  }
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "2028d7e6f62be088c6e7"; }
/******/ }();
/******/ 
/******/ }
);