...
const main = async err => {
  ...
  function moverMenu() {
    //let menuClone = [...items];
    const menuDesplazado = document.getElementsByClassName('fake-ceramic');
    ...
    console.log(menuDesplazado[0]);

    for (let i = 0; i < menuDesplazado[0].length; i++) {
      console.log(menuDesplazado[0][i]);

      if (menuDesplazado[0][i].classList.contains('current-menu-item')) {
        menuDesplazado[0][i].classList.add('hidden');
      } else {
        menuDesplazado[0][i].classList.add('flex');
      }
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "96adcfbbb177230775ec"; }
/******/ }();
/******/ 
/******/ }
);