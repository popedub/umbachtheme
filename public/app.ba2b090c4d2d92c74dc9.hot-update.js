  ...
  if (document.body.classList.contains('page-template-template-classes')) {
    ...
    if (items) {
      ...
      if (window.innerWidth < 768) {
        let menu = [];
        const menuDesplazado = document.createElement('ul');
        menuDesplazado.classList.add('nav-ceramics');
        items.forEach(itemMenu => {
          if (!itemMenu.classList.contains('current-menu-item')) {
            menu.push(itemMenu.classList.add('flex'));
            itemMenu.classList.add('hidden', 'lg:flex'); // menu.push(itemMenu);
          } else {
            itemMenu.classList.add('flex');
          }
        }); // console.log(menu)

        menuDesplazado.prepend(...menu); // console.log(menuDesplazado)

        var foot = document.getElementById('footer');
        foot.appendChild(menuDesplazado);
      }
    }
  }
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "c2f7deceb4a440251d1f"; }
/******/ }();
/******/ 
/******/ }
);