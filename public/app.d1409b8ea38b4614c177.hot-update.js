...
const main = async err => {
  ...
  if (document.body.classList.contains('single')) {
    ...
    for (let i = 0; i < itemsGaleria.length; i++) {
      getAltura(itemsGaleria[i]);
      window.addEventListener('resize', getAltura(itemsGaleria[i]));
    }

    getAltura(galeria);
    window.addEventListener('resize', getAltura(galeria));
    var mySwiper = new swiper_bundle__WEBPACK_IMPORTED_MODULE_3__["default"]('.swiper', {
      // configure Swiper to use modules
      // Optional parameters
      slidesPerView: 1,
  ...
  function getAltura(galeria) {
    ...
    let alt = elemento[0].offsetHeight; // let footer = document.getElementById('footer');
    //caculamos la altura y le restamos el header, el page-header y el footer le damos la altura al main
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "363d145234718349c2bb"; }
/******/ }();
/******/ 
/******/ }
);